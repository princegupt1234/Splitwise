...
export const Avatar = ({ name, size = 'md', className = '' }) => {
  ...
  return (
    <div className={`${sizes[size]} ${getAvatarColor(name)} rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 ring-2 ring-white/10 ${className}`}>
      {getInitials(name)}
    </div>
  );
};
...
export const Spinner = ({ size = 'md', className = '' }) => {
  ...
  return (
    <div className={`${s[size]} rounded-full animate-spin ${className}`}
      style={{ borderColor: 'rgba(255,255,255,0.12)', borderTopColor: '#6574f3' }} />
  );
};
...
export const EmptyState = ({ icon, title, description, action }) => (
  <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
    ...
    {description && <p className="text-sm leading-relaxed mb-6 max-w-xs" style={{ color: '#4a4d5e' }}>{description}</p>}
    {action}
  </div>
);
      ...
      <div className="relative w-full max-w-md mx-4 mb-4 sm:mb-0 rounded-3xl overflow-hidden shadow-2xl"
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <button onClick={onClose}
            ...
            style={{ background: 'rgba(255,255,255,0.06)', color: '#6a6d80' }}
            onMouseEnter={e => e.currentTarget.style.color = '#fff'}
            onMouseLeave={e => e.currentTarget.style.color = '#6a6d80'}
          >✕</button>
        </div>
...
export const StatCard = ({ label, value, icon, colorClass }) => (
  <div className="card p-4 flex flex-col gap-2">
    ...
    <p className="text-xs font-medium" style={{ color: '#4a4d5e' }}>{label}</p>
  </div>
);
